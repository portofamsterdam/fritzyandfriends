...
public class APXPricesService implements IAPXPricesService {
    ...
    private static final DateTimeFormatter DATE_TIME_PARSER = DateTimeFormatter.ofPattern("yyyy-MM-dd'T'HH:mm'Z'").withZone(ZoneId.of("UTC"));
    private APXPriceRetriever apxPriceRetriever;
    private TransparencyPlatformClient client;
    
    public static class NoPricesAvailableException extends RuntimeException {
	...
	@Override
	public double getPricePerkWh(Instant requestedDateTime) {
	...
	private double getSinglePrice(Instant requestedDateTime, PublicationMarketDocument prices) {
        for (TimeSeries timeSeries: prices.getTimeSeries()) {
        	...
        	long factor;
        	StandardUnitOfMeasureTypeList unit = StandardUnitOfMeasureTypeList.fromValue(timeSeries.getPriceMeasureUnitName());
        	switch (unit) {
        	case GWH:
        		factor = 1000 * 1000;
				break;
        	...
        	for (SeriesPeriod period: timeSeries.getPeriod()) {
        		...
        		if (!requestedDateTime.isBefore(periodStart) && requestedDateTime.isBefore(periodEnd)) {
        			LOG.debug("requestedDatetime {} is in current period {}-{}", requestedDateTime, periodStart, periodEnd);
        			...
        			long offset = Duration.between(periodStart, requestedDateTime).getSeconds() / resolution.getSeconds();
        			for (Point point : period.getPoint()) {
        				if (point.getPosition() == offset + 1) {
        					LOG.debug("For requested time {} found pricepoint at position {}: value {} {} per {}", 
        							requestedDateTime, point.getPosition(), point.getPriceAmount().doubleValue(), timeSeries.getCurrencyUnitName(), unit );
